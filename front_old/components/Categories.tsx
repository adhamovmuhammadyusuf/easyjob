...
const Categories: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {      try {
        setLoading(true);
        console.log('Fetching categories from:', `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/categories/`);
        const data = await getCategories();
        console.log('Categories data:', data);
        if (Array.isArray(data) && data.length > 0) {
          setCategories(data);
        } else {
          console.warn('Categories data is not an array or empty:', data);
          setCategories([]);
        }
        setError(null);
      } catch (err: any) {
          ...
          {categories.map((category) => (
            ...
            >
              ...
              <div className="relative flex items-center justify-between">
                ...
                </div>                <div className="text-4xl transform group-hover:scale-110 transition-transform duration-300">
                  📁
                </div>
              </div>
            </Link>
